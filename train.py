...
if __name__ == '__main__':
    ...
    experiment_name = f'TMIF_' + datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    # define model
    mul_net = CrossTransIngration(num_classes=args.numclasses, vocab_size=args.num_words_to_keep, embedding_size=args.embedding_dim)

    creterion = torch.nn.CrossEntropyLoss()

    # define optimizer
    raw_optimizer = torch.optim.SGD(list(mul_net.parameters()), lr=args.lr, momentum=args.momentum, weight_decay=args.wd)
    ...
    for epoch in range(0, args.max_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            img_logits, res_logits, text_logits, mul_logits = mul_net(imgs, texts)
            # calculate loss
    ...
    all_output = ['{', "runtime:" + str(end-start), "\"train_acc_list\": " + str(train_acc_list), "\"test_acc_list\": " +
